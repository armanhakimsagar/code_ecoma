...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalShopping      = Order::paid()->delivered()->selectRaw('SUM(total_amount - shipping_charge)')->value('total_shopping');
        $totalTransaction   = Transaction::where('user_id', $user->id)->count();
        $totalOrders        = Order::valid()->where('user_id', $user->id)->count();
        $countries          = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalShopping', 'totalTransaction', 'countries', 'totalOrders'));
    }
