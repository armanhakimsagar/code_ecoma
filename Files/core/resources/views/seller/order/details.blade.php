...
@section('panel')
                    ...
                    @if ($suborder->order->status != Status::ORDER_CANCELED && !in_array($suborder->status, [Status::SUBORDER_DELIVERED, Status::SUBORDER_REJECTED]))
                        <div class="text-end">
                            @if ($suborder->status == Status::SUBORDER_PENDING)
                                <button type="button" class="btn btn-outline--success confirmationBtn" data-question="@lang('Are you sure to mark the order as processing?')" data-action="{{ route('seller.order.mark.as.processing', $suborder->id) }}"><i class="las la-check-double"></i>@lang('Mark As Processing')</button>
                            @elseif($suborder->status == Status::SUBORDER_PROCESSING)
                                <button type="button" class="btn btn-outline--success confirmationBtn" data-question="@lang('Are you sure to mark the order as ready to pickup?')" data-action="{{ route('seller.order.mark.as.ready.to.pickup', $item->id) }}"><i class="las la-check-double"></i>@lang('Mark As Ready to Pickup')</button>
                            @endif
                        </div>
                    @endif
                </div>
    ...
    @if ($suborder->order->status != Status::ORDER_CANCELED && in_array($suborder->status, [Status::SUBORDER_PENDING, Status::SUBORDER_PROCESSING]))
