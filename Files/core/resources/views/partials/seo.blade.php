@if ($seo)
    ...
    <meta itemprop="image" content="{{ @$seoContents->image ?? getImage(getFilePath('seo') . '/' . $seo->image) }}">
    {{-- <!-- Facebook Meta Tags --> --}}
    ...
    <meta property="og:image" content="{{ @$seoContents->image ?? getImage(getFilePath('seo') . '/' . $seo->image) }}">
    <meta property="og:image:type" content="image/{{ pathinfo(getImage(getFilePath('seo')) . '/' . $seo->image)['extension'] }}">
    @php $socialImageSize = explode('x', @$seoContents->image_size ?? getFileSize('seo')) @endphp
